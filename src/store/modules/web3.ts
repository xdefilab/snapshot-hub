...
import { supportIds, getConfigByNameAndChainId } from '@/configs';
import xdexStreamAbi from 'xdefi-assets/abi/XdexStream.json';
import xhalflifeAbi from 'xdefi-assets/abi/XHalfLife.json';
...
const state = {
  ...
  balances: {},
  blockNumber: 0,
  currentChainId: Number(localStorage.getItem('activeChainId') || supportIds[0])
};
...
const actions = {
  ...
  fetchVotePowerBalance: async ({ rootState }) => {
    const chainId = rootState.web3.injectedChainId;
    const addresses = getConfigByNameAndChainId(chainId, 'addresses');
    const address = rootState.web3.account;
    const { XdexStream, xHalfLife } = addresses;
    const streamContract = new Contract(XdexStream, xdexStreamAbi, web3);
    const halflifeContract = new Contract(xHalfLife, xhalflifeAbi, web3);

    const streamId = await streamContract.getStreamId(address, 0);
    if (streamId) {
      const formatStreamId = Number(formatUnits(streamId, 0));
      const { remaining } = await halflifeContract.balanceOf(formatStreamId);
      const votePower = formatUnits(remaining);
      return votePower;
    } else {
      return 0;
    }
